...
from net.PromptFFTNetBasedTurb import PromptFFTNet
...
class PromptFFTNetModel(pl.LightningModule):
    def __init__(self):
        super().__init__()
        self.net = PromptFFTNet(decoder=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_path', type=str, default="/home/uib42091/Muthu/heatChamberTest/synthetic/",
                        help='save path of test images, can be directory or an image')
    parser.add_argument('--output_path', type=str, default="Demo3HeatChamber1/", help='output save path')
    parser.add_argument('--ckpt_name', type=str, default="PromptFFTNetModel.ckpt", help='checkpoint save path')
    ...
    ckpt_path = "/home/uib42091/Muthu/PromptFFTNet/train_ckpt/" + opt.ckpt_name
    ...
    net = PromptFFTNetModel.load_from_checkpoint(ckpt_path).to(device)
    net.eval()
    ...
    print('Start testing...')
    with torch.no_grad():
        for ([clean_name], degrad_patch, degraded_id) in tqdm(testloader):
            ...
            folder_name = os.path.basename(os.path.dirname(degraded_id[0]))
            output_folder = os.path.join(opt.output_path, folder_name)
            os.makedirs(output_folder, exist_ok=True)
            save_path = os.path.join(output_folder, clean_name[0] + '.png')
            save_image_tensor(restored, save_path)
            # save_image_tensor(restored, opt.output_path + folder_name + '/' + clean_name[0] + '.png')
